...
void main() {
  late OnAiringTvBloc onTheAirNowBloc;
  ...
  late MockGetTvDetailail mockGetTvDetailail;
  ...
  setUp(() {
    mockGetTvDetailail = MockGetTvDetailail();
    ...
    onTheAirNowBloc = OnAiringTvBloc(mockGetOnTheAirTv);
  ...
  group('Get now playing movies', () {
    ...
    blocTest(
      ...
      act: (OnAiringTvBloc bloc) => bloc.add(FetchNowPlayingTvSeries()),
    ...
    blocTest(
      ...
      verify: (OnAiringTvBloc bloc) => verify(mockGetOnTheAirTv.execute()),
    );
  });
  ...
  group('Get Recommended movies', () {
    ...
    blocTest(
      ...
      act: (TvSeriesRecommendationBloc bloc) =>
          bloc.add(FetchTvSeriesRecommendation(tId)),
      wait: const Duration(milliseconds: 500),
      expect: () => [TvSeriesLoading(), TvSeriesHasData(tTvList)],
    );
  ...
  group('Get Details movies', () {
    ...
    blocTest(
      'should emit[loading, movieHasData] when data is gotten succesfully',
      build: () {
        when(mockGetTvDetailail.execute(tId))
    ...
    blocTest(
      'Should emit [Loading, Error] when get search is unsuccessful',
      build: () {
        when(mockGetTvDetailail.execute(tId)).thenAnswer(
            (realInvocation) async => Left(ServerFailure('Server Failure')));
        return detailTvBloc;
      },
      act: (TvSeriesDetailBloc bloc) => bloc.add(FetchDetailTvSeries(tId)),
      ...
      verify: (bloc) => verify(mockGetTvDetailail.execute(tId)),
    );
  });
    ...
    group('Watchlist Movie', () {
      ...
      group('Load Watchlist Movie', () {
        blocTest(
          ...
          act: (WatchlistTvSeriesBloc bloc) =>
              bloc.add(WatchlistTvSeriesStatus(tId)),
          wait: const Duration(milliseconds: 500),
          expect: () => [
            TvSeriesLoading(),
        ...
        blocTest(
          ...
          build: () {
            when(mockSaveTvWatchlist.execute(testTvDetail)).thenAnswer(
                (_) async =>
                    Right(WatchlistTvSeriesBloc.watchlistAddSuccessMessage));
            return watchlistTvBloc;
          },
          act: (WatchlistTvSeriesBloc bloc) =>
          ...
          expect: () => [
            ...
            WatchlistTvSeriesMessage(
                WatchlistTvSeriesBloc.watchlistAddSuccessMessage),
          ],
